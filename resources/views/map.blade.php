...
<body>
    <section class="flex flex-row">
        ...
        <div class="flex flex-col md:flex-row">
            <div class="order-last md:order-first w-full md:w-1/3">
                {{-- modal --}}
                ...
                {{-- end modal --}}
                <div id="lokasi" class="ml-4">
                    @foreach ($daftar as $item)
                        <div class="mt-2 h-30 border-2 border-gray-500 rounded-lg pr-4">
                            <img style="width: 1000px !important;" class="px-1 py-2"
        ...
        success: function(response) {
            const myLayer = L.geoJSON(response, {
                onEachFeature: function(feature, layer) {
                    layer.bindPopup(
                        `<h1><b>
        <center> ${feature.properties.name} </center></b> <center> Kec. ${feature.properties.kecamatan}</center> <center> Desa ${feature.properties.desa} </center> 
        </h1>`
                    )
                },
            }).on('click', function(e) {
    ...
    function getDetailLokasi(id) {
        $.ajax({
            ...
            success: function(response) {
                ...
                lokasi.innerHTML = ` <div class="mt-2 h-30 border-2 border-gray-400 rounded-lg pr-2">
                                    <img style="width: 1000px !important;" class="min-h-72 object-cover px-1 py-2 ml-2" src="/storage/${ response.img }" >
